        ...
        <>
          ...
          >
            ...
            <p className="mt-2 text-xs text-slate-400">
              «Выручка (по заказам)» — сумма на момент заказа, ещё до выкупа. Она НЕ равна «Выручка (выкуп)» в блоке
              «Маржа» ниже и не равна «Выручке» в кабинете Ozon — это разные моменты подсчёта, не ошибка.
            </p>
          </DashboardSection>
                  ...
                  <Stat
                    ...
                    title={
                      dashboard.logistics.other_services_top_item_name
                        ? `Крупнейшая статья за пересекающуюся неделю: ${dashboard.logistics.other_services_top_item_name} — ${fmtRub(dashboard.logistics.other_services_top_item_rub)}`
                        : undefined
                    }
                  />
                </div>
                <p className="mt-2 text-xs text-slate-400">{dashboard.logistics.period_note}</p>
              </>
            ) : (
...
function Stat({ label, value, preliminary, title }: { label: string; value: string; preliminary?: boolean; title?: string }) {
  return (
    <div className="rounded-md border border-slate-200 bg-slate-50 p-3" title={title}>
