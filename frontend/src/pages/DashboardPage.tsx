            ...
            {dashboard.logistics.periods_summed > 0 ? (
              <>
                ...
                <p className="mt-2 text-xs text-slate-400">
                  ...
                  статьи услуг Ozon, распознанные по названию (например, «...TemporaryStorage...», «Fines...»);
                  «Услуги партнёров» — эквайринг и страхование товара (доставка/упаковка/размещение партнёрами
                  — статьи с ещё не подтверждённым названием — сюда пока не попадают); «Услуги FBO» —
                  ...
                  не будет угадано в одну из категорий выше. Проверено на реальном аккаунте (2026-09-19): в
                  «Прочие услуги» чаще всего попадают всего 2-3 статьи, не мусор — «MarketplaceServiseItemAgencyFee
                  ForSale» (агентская комиссия за продажу — самая крупная и волатильная, может быть и расходом, и
                  доходом в разные недели) и «MarketplaceServiseItemPointsAwarded» (баллы, начисленные покупателям
                  по программе лояльности — всегда положительная сумма, уменьшает «Прочие услуги» по модулю); у
                  вашего магазина может быть иначе, если Ozon использует другие статьи. Ozon группирует эти цифры
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-xs text-slate-400">
        ...
        {margin.revenue_from_accrual ? (
          <>
            {" "}«Выручка (выкуп)» — точная сумма из отчёта Ozon о реализации за каждый день периода (POST
            /v1/finance/realization/by-day), а не оценка по заказам — совпадает с личным кабинетом Ozon до копейки.
          </>
        ) : (
          <>
            {" "}Пометка «предварительно» на «Выручке (выкуп)»: точные данные из отчёта Ozon о реализации собраны не
