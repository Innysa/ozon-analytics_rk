...
def sync_ozon_products(
    ...
    """Manual trigger for the same product/stock sync that also runs
    automatically every night (see app.services.scheduler) — for when a
    seller needs fresher stock numbers sooner than the next scheduled run."""
    creds = db.query(OzonCredentials).filter(OzonCredentials.store_id == ctx.store_id).first()
    if not creds:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Для магазина не заданы ключи Ozon")

    run = sync_store_products(db, ctx.store_id, creds, initiated_by_user_id=user.id)
    return _serialize(run)
