"""Wires an in-process APScheduler job that runs the automatic per-product
funnel sync (app.services.product_analytics_daily_sync_service) once a day
...
def start_product_analytics_daily_statistics_scheduler() -> BackgroundScheduler | None:
    ...
    _scheduler.add_job(
        ...
        CronTrigger(
            hour=settings.PRODUCT_ANALYTICS_STATS_SCHEDULER_HOUR_UTC,
            minute=settings.PRODUCT_ANALYTICS_STATS_SCHEDULER_MINUTE_UTC,
        ),
        id="product_analytics_daily_statistics_sync",
        replace_existing=True,
        misfire_grace_time=3600,
    )
    _scheduler.start()
    logger.info(
        "Аналитика товаров: планировщик автосбора запущен (ежедневно в %02d:%02d UTC)",
        settings.PRODUCT_ANALYTICS_STATS_SCHEDULER_HOUR_UTC, settings.PRODUCT_ANALYTICS_STATS_SCHEDULER_MINUTE_UTC,
    )
    return _scheduler
