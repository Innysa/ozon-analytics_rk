...
from app.core.moscow_time import utc_to_moscow_date
...
def _parse_in_process_at(value: str | None) -> date | None:
    ...
    MOSCOW calendar day, not raw UTC day (see app.core.moscow_time) —
    Ozon's own cabinet reports by Moscow local day (same class of bug
    already confirmed and fixed on the frontend, see isoDate()'s own
    docstring in DashboardPage.tsx/RnpPage.tsx: "converts to UTC first,
    which silently shifts the day back for any timezone ahead of UTC"). A
    posting at 21:30 UTC on day N is 00:30 MSK on day N+1 — bucketing by
    raw UTC date puts it a day EARLY relative to Ozon's own per-day counts,
    which is exactly the kind of systematic day-boundary mismatch a seller
    would notice comparing our per-day order count against the cabinet's."""
    if not value:
        return None
    try:
        return utc_to_moscow_date(datetime.fromisoformat(value.replace("Z", "+00:00")))
    except ValueError:
        return None
