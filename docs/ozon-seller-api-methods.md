...
- `/v1/finance/realization/posting` — официальное название в docs.ozon.ru
  (раздел «Финансовые отчёты», подтверждено скриншотом 2026-09-19):
  **«Позаказный отчёт о реализации товаров»**. Контракт (поля запроса/
  ответа) НЕ подтверждён — метод ни разу не вызывался. Кандидат на поиск
  «Услуги доставки» (см. README, этот поиск через `realization/by-day`
  закрыт отрицательно — возможно, по-заказная детализация несёт то, чего
  нет в по-дневной).
