...
- `/v1/finance/realization/posting` — официальное название в docs.ozon.ru
  (раздел «Финансовые отчёты»): **«Позаказный отчёт о реализации
  товаров»**. Контракт ПОДТВЕРЖДЁН 2026-09-20 (пользователь скопировала
  официальную документацию, метод НЕ вызывался вживую): тело запроса
  `{"month": int, "year": int}` — за ЦЕЛЫЙ МЕСЯЦ, не за день. Ответ —
  `{"header": {...реквизиты документа...}, "rows": [...]}`, строка на
  ЗАКАЗ (есть `order.posting_number`/`created_date`), но САМИ ПОЛЯ СТРОКИ
  — `commission_ratio`, `delivery_commission`, `return_commission`
  (те же под-поля: amount/bonus/commission/compensation/price_per_instance/
  quantity/standard_fee/bank_coinvestment/stars/pick_up_point_coinvestment/
  total), `item`, `row_number`, `seller_price_per_instance` — В ТОЧНОСТИ
  СОВПАДАЮТ с уже перебранными полями `/v1/finance/realization/by-day`
  (см. ниже, поиск «Услуги доставки» там закрыт отрицательно). Значит
  этот метод НЕ поможет с «Услуги доставки» — та же нарезка полей, только
  за месяц и с привязкой к отправлению вместо дня. **Поиск «Услуги
  доставки» через методы realization/accrual окончательно закрыт.**
...
- Кабинетный отчёт «Начисления» (Финансы → Начисления → «Скачать отчёт»,
  XLSX; колонки «Группа услуг»/«Тип начисления» — ТОЧНО те же группы, что
  видно на экране «Начисления») — **НЕТ подтверждённого метода API с таким
  же разрезом (день × группа × тип)**. Проверено 2026-09-22 на реальном
  файле: поля `/v1/finance/realization/posting` — другая, более грубая
  схема (per-order, не per-day/group/type — см. запись выше, "Поиск «Услуги
  доставки»... окончательно закрыт"), `/v1/finance/accrual/postings`
  требует конкретных номеров отправлений (не диапазон дат), а
  `/v1/finance/cash-flow-statement/list` даёт только смешанную статью
  `services`/`others` с английскими внутренними именами Ozon, без родных
  русских «Группа услуг»/«Тип начисления» и без реальной календарной даты
  на строку (только недельные периоды Ozon) — качественно другой, менее
  точный источник (см. README, «Логистика и услуги», абзац от
  2026-09-22). Используется через ручную (пере)загрузку —
  `app.services.accrual_report_import`, `AccrualReportDailyStatistic`,
  `POST /api/stores/{id}/dashboard/upload-accruals` — тот же паттерн, что у
  «Локальность продаж» для локализации по товарам
...
### Официальные названия из docs.ozon.ru → «Финансовые отчёты» (скриншот 2026-09-19)

Подтверждённый пользователем полный список раздела «Финансовые отчёты» в
официальной документации (не путать со списком методов ключа выше — тут
только человекочитаемые названия, контракты по-прежнему не подтверждены,
кроме уже помеченных «ИСПОЛЬЗУЕТСЯ»):

- Отчёт о реализации товаров (версия 2) — `POST /v2/finance/realization`
- Позаказный отчёт о реализации товаров — `POST /v1/finance/realization/posting`
- Список транзакций — `POST /v3/finance/transaction/list` (см. ниже — устарел)
- Суммы транзакций — `POST /v3/finance/transaction/totals` (не в списке методов ключа выше — новый кандидат, не пробован)
- Реестр продаж юридическим лицам — `POST /v1/finance/document-b2b-sales`
- Реестр продаж юридическим лицам в JSON-формате — `POST /v1/finance/document-b2b-sales/json`
- Отчёт о взаиморасчётах — `POST /v1/finance/mutual-settlement`
- Отчёт о выкупленных товарах — `POST /v1/finance/products/buyout`
- Отчёт о компенсациях — `POST /v1/finance/compensation`
- Отчёт о декомпенсациях — `POST /v1/finance/decompensation`

`/v1/finance/accrual/by-day`, `/v1/finance/accrual/postings`,
`/v1/finance/accrual/types`, `/v1/finance/realization/by-day`,
`/v1/finance/balance`, `/v1/finance/cash-flow-statement/list` в этом
разделе документации НЕ показаны на присланных скриншотах — either в
другом разделе сайдбара, либо страница была прокручена не до конца;
уточнить при следующей проверке, если понадобится.
